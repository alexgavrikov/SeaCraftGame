...
//	std::getline(html_file, file_line_third, kGetlineEnd);
...
//	std::cout<<"j"<<(file_line.substr(file_line.size()-1, 1) == "\n")<<"j"<<std::endl;
//	std::cout<<"j"<<(file_line.substr(file_line.size()-2, 2) == "\r\n")<<"j"<<std::endl;
//	std::cout<<"j"<<(file_line.substr(file_line.size()-1, 1) == "\r")<<"j"<<std::endl;
//	std::cout<<"j"<<(file_line_second.substr(0, 1) == "\r")<<"j"<<std::endl;
//	std::cout<<"j"<<(file_line_second.substr(0, 1) == "\n")<<"j"<<std::endl;
	...
	while (true) {
		...
		if (query.message.find("GET / HTTP") != std::string::npos) {
			...
			const std::string answer_to_get_query_for_icon = "HTTP/1.1 200 OK" + kLineEnd
					+ "Content-Length: 3" + kLineEnd + "Content-Type: text/html" + kLineEnd + kLineEnd
					+ "OKK";
			send(query.sock, answer_to_get_query_for_icon.c_str(), sizeof(answer_to_get_query_for_icon),
					0);
		}
	}
}
...
bool Server::LoopOfPreprocessingFromOneSocket(
	...
	while (true) {
		...
		if (current_unfinished_message.substr(0, 3) == "GET") {
			std::string ff = kLineEnd + kLineEnd;
			size_t end_pos = current_unfinished_message.find(ff);
			std::cout << "qq"
					<< current_unfinished_message.substr(current_unfinished_message.size() - 2, 2) << "qq"
					<< std::endl;
			std::cout << "qq"
					<< current_unfinished_message.substr(current_unfinished_message.size() - 4, 4) << "qq"
					<< std::endl;
			std::cout << "qq"
					<< (current_unfinished_message.substr(current_unfinished_message.size() - 2, 2) == ff)
					<< "qq" << std::endl;
			std::cout << "qq"
					<< (current_unfinished_message.substr(current_unfinished_message.size() - 4, 4)
							== "\r\n\r\n") << "qq" << std::endl;
			std::cout << "d" << (ff == "\n\n") << "d" << std::endl;
			std::cout << "w" << (ff == "\r\n\r\n") << "w" << std::endl;
			std::cout << "q" << current_unfinished_message << "q" << std::endl;
			std::cout << "vwe" << kLineEnd << "ee" << (end_pos == std::string::npos) << std::endl;
			if (end_pos != std::string::npos) {
				package = current_unfinished_message.substr(end_pos + 2 * kLineEndLen);
				queue_of_GET_queries.enqueue(
						QueryAndSocket(current_unfinished_message.substr(0, end_pos + 2 * kLineEndLen),
								source_socket));
			...
			size_t pos2 = current_unfinished_message.find(kLineEnd + kLineEnd);
			if (pos == std::string::npos || pos2 == std::string::npos) {
				continue;
			}
			size_t pos3 = current_unfinished_message.find(kLineEnd, pos);
			...
			std::string post_query = current_unfinished_message.substr(0,
					(pos2 + 2 * kLineEndLen) + content_length);

			//Skipping HTTP header, its end is indicated by an empty line
			int begposition_of_message_itself = post_query.find(kLineEnd + kLineEnd) + 2 * kLineEndLen;
			...
			package = current_unfinished_message.substr((pos2 + 2 * kLineEndLen) + content_length);
			current_unfinished_message.clear();
		}
	}
}
...
void Server::ParseData(const char* buf, int size, Clients::iterator client_iterator) {
...
bool Server::RecieveShips(const char* buf, int size, Clients::iterator client_iterator) {
...
//  bool user_wants_random = false;
//  if (std::string(buf, 15) != "shipping:random") {
...
bool Server::RecieveStep(const char* buf, int size, Clients::iterator client_iterator) {
...
//  std::cout << "result of shooting: " << result_of_shooting << std::endl;
	...
	case TClient::MISS: {
		...
		std::string message_for_opponent = "field1:miss"
				+ stream_for_message_ending_if_not_killed.str();
	...
	case TClient::HALF: {
		...
		std::string message_for_opponent = "field1:half"
				+ stream_for_message_ending_if_not_killed.str();
		client_iterator->PrepareMessage(message_for_client);
		client_iterator->opponent_->PrepareMessage(message_for_opponent);
		break;
