...
void TClient::SendMessage() {
  ...
  whole_message << message_itself.size() << kHttpLineEnd << "Content-Type: text/html"
      << kHttpHeaderMessageDelimiter << message_itself;
...
bool TClient::CorrectShips() const {
  ...
  return number_of_ships[0] == 4 && number_of_ships[1] == 3 && number_of_ships[2] == 2
      && number_of_ships[3] == 1;
}
...
Coordinate TClient::FindBeginOfShip(const Coordinate ship_begin) const {
  ...
  if (next_state.first >= 0 && ships_[next_state.first][next_state.second] != TClient::WATER) {
    while (next_state.first >= 0 && ships_[next_state.first][next_state.second] != TClient::WATER) {
  ...
  if (next_state.second >= 0 && ships_[next_state.first][next_state.second] != TClient::WATER) {
    while (next_state.second >= 0 && ships_[next_state.first][next_state.second] != TClient::WATER) {
...
Coordinate TClient::FindEndOfShip(const Coordinate ship_begin) const {
  // TRY TO GO DOWN
  auto next_state = std::make_pair(ship_begin.first + 1, ship_begin.second);
  if (next_state.first < 10 && ships_[next_state.first][next_state.second] != TClient::WATER) {
    while (next_state.first < 10 && ships_[next_state.first][next_state.second] != TClient::WATER) {
      next_state = std::make_pair(next_state.first + 1, next_state.second);
    }
    return std::make_pair(next_state.first - 1, next_state.second);
  }
  // TRY TO GO RIGHT
  next_state = std::make_pair(ship_begin.first, ship_begin.second + 1);
  if (next_state.second < 10 && ships_[next_state.first][next_state.second] != TClient::WATER) {
    while (next_state.second < 10 && ships_[next_state.first][next_state.second] != TClient::WATER) {
      next_state = std::make_pair(next_state.first, next_state.second + 1);
    }
    return std::make_pair(next_state.first, next_state.second - 1);
  }
...
bool TClient::IsInGrid(const Coordinate coordinate) const {
  return -1 < coordinate.first && -1 < coordinate.second && coordinate.first < 10
      && coordinate.second < 10;
}

bool TClient::IsInShip(const Coordinate coordinate, const Coordinate ship_begin,
    const Coordinate ship_end) const {
  if (ship_begin.first == ship_end.first) {  // Horizontal ship
    return coordinate.first == ship_begin.first && coordinate.second >= ship_begin.second
        && coordinate.second <= ship_end.second;
  } else { // Vertical ship
    return coordinate.second == ship_begin.second && coordinate.first >= ship_begin.first
  ...
  while (current_state <= ship_end) {
    for (int x_diff = -1; x_diff <= 1; ++x_diff) {
      for (int y_diff = -1; y_diff <= 1; ++y_diff) {
        ...
        if (IsInGrid(current_neighbour) && !IsInShip(current_neighbour, ship_begin, ship_end)) {
...
// Returns HALF if you strike DEAD piece not killing
